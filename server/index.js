...
app.post("/saveCV", async(req, res) => {
        const { CVID, Fname, Lname, AvatarUser, Email, dob, phone, Address, Country, Bio, Skills, Hobbies, Educations, Employments, CVThumbnail, CVImage } = req.body
        try {
            let result = await db.saveCVInfo(CVID, Fname, Lname, AvatarUser, Email, dob, phone, Address, Country, Bio, Skills, Hobbies, Educations, Employments, CVThumbnail, CVImage);
            res.send(CVID);
        } catch (err) {
            console.log(err)
        }
